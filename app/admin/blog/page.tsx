...
export default function BlogManagementPage() {
  ...
  useEffect(() => {
    ...
    // 从localStorage加载AI配置
    const savedAiEnabled = localStorage.getItem('blog_ai_enabled');
    if (savedAiEnabled !== null) {
      setAiEnabled(JSON.parse(savedAiEnabled));
    }

    const savedAiPrompt = localStorage.getItem('blog_ai_prompt');
    if (savedAiPrompt) {
      setAiPrompt(savedAiPrompt);
    }

    // 加载模板数组
    const savedTemplates = localStorage.getItem('blog_templates');
    if (savedTemplates) {
    ...
    // 加载随机模板开关状态
    const savedRandomTemplateEnabled = localStorage.getItem('blog_random_template_enabled');
    if (savedRandomTemplateEnabled !== null) {
      setRandomTemplateEnabled(JSON.parse(savedRandomTemplateEnabled));
    }

    const savedPublishStrategy = localStorage.getItem('blog_publish_strategy');
    if (savedPublishStrategy === 'auto' || savedPublishStrategy === 'manual') {
      setPublishStrategy(savedPublishStrategy);
    }

    // 加载发布设置
    const savedManualEnabled = localStorage.getItem('blog_manual_enabled');
    if (savedManualEnabled !== null) {
      setManualEnabled(JSON.parse(savedManualEnabled));
    }
    const savedScheduleEnabled = localStorage.getItem('blog_schedule_enabled');
    if (savedScheduleEnabled !== null) {
      setScheduleEnabled(JSON.parse(savedScheduleEnabled));
    }
    const savedScheduleTime = localStorage.getItem('blog_schedule_time');
    if (savedScheduleTime) {
      setScheduleTime(savedScheduleTime);
    }
    const savedRandomEnabled = localStorage.getItem('blog_random_enabled');
    ...
    if (savedPosts) {
      ...
      // 为旧数据添加status字段
      const postsWithStatus = parsedPosts.map((post: any) => ({
        ...post,
...
if (typeof window !== 'undefined') {
  ...
  // 保存AI配置
  useEffect(() => {
    localStorage.setItem('blog_ai_enabled', JSON.stringify(aiEnabled));
  }, [aiEnabled]);

  // 保存模板数组
  useEffect(() => {
    localStorage.setItem('blog_templates', JSON.stringify(templates));
  }, [templates]);

  // 保存随机模板开关状态
  useEffect(() => {
    localStorage.setItem('blog_random_template_enabled', JSON.stringify(randomTemplateEnabled));
  ...
  useEffect(() => {
    localStorage.setItem('blog_ai_prompt', aiPrompt);
  }, [aiPrompt]);

  useEffect(() => {
    localStorage.setItem('blog_publish_strategy', publishStrategy);
  }, [publishStrategy]);

  // 保存发布设置
  useEffect(() => {
    localStorage.setItem('blog_manual_enabled', JSON.stringify(manualEnabled));
  }, [manualEnabled]);

  useEffect(() => {
    localStorage.setItem('blog_schedule_enabled', JSON.stringify(scheduleEnabled));
  }, [scheduleEnabled]);

  useEffect(() => {
    localStorage.setItem('blog_schedule_time', scheduleTime);
  }, [scheduleTime]);
  ...
  const handleUnpublishPost = (id: string) => {
    const updatedPosts = posts.map(post => 
      post.id === id ? { ...post, status: 'pending' as const } : post
    );
    setPosts(updatedPosts);
    localStorage.setItem('blogPosts', JSON.stringify(updatedPosts));
  };
  ...
  const handleGenerateBlog = async () => {
    ...
    try {
      ...
      // 更新最后生成日期
      localStorage.setItem('blog_last_generate_date', today);
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {showAddForm && (
              <div className="mb-6 p-4 border border-gray-200 rounded-md">
                <h2 className="text-lg font-medium text-gray-900 mb-4">添加新博客</h2>
            ...
            {posts.filter(post => post.status === activeTab).length === 0 && (
              <div className="text-center py-12 text-gray-500">
                {activeTab === 'pending' ? '暂无待审核博客' : '暂无已发布博客'}
              </div>
            )}
          </div>
        </div>
      ...
      {showEditModal && editingPost && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-900">编辑博客</h3>
              <button
